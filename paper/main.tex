...
Under Black--Scholes, a single constant $\sigma$ must reconcile prices across strikes and maturities. Empirically, equity index options
exhibit strike--dependent implied volatilities (smile/smirk) and nontrivial term structure, producing systematic deviations when a
constant--volatility model is imposed. These findings motivate richer dynamics such as local volatility or stochastic volatility.
Future work will implement the Heston model, calibrate via characteristic functions/Fourier pricing, and compare fit and stability to
the Black--Scholes baseline on the same SPX chains.
